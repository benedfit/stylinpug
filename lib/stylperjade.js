...
function process(cssFiles, jadeFiles, options, cb) {
    ...
    function generateFileReport(type, fileType, collection) {
      ...
      if (collection.length > 0) {
        ...
        collection.forEach(function (className) {
          ...
          className.locations.forEach(function (location) {
            if (location.file) {
    ...
    switch (obj.type) {
      case 'selector':
        obj.selectors.forEach(function (selector) {
          processCssClasses(selector, position)
        })
        break
      case 'ruleSet':
      ...
      case 'rule':
        if (obj.classNames) {
          obj.classNames.forEach(function (className) {
            pushUnique(cssClasses, className, position.source, position.start.line)
          })
        }
  ...
  function processCssFile(file, cb) {
    ...
    fs.readFile(file, 'utf8', function (err, data) {
      ...
      var souremapFile = parseSourcemapFile(file)

      cssParser.parse(data, { silent: true, source: souremapFile }).stylesheet.rules.forEach(function (rule) {
        processCssRule(rule)
      })
  ...
  function processJadeFile(file, cb) {
    ...
    fs.readFile(file, 'utf8', function (err, data) {
      ...
      jadeWalk(jadeParser(jadeLexer(data), file), function after(node) {
        if (node.type === 'Tag') {
  ...
  function pushUnique(collection, value, file, line, column) {
    ...
    classNames.forEach(function (className) {
      var existing = _.find(collection, 'name', className)
