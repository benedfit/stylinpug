...
function process(cssFiles, jadeFiles, options, cb) {
      ...
      if (collection.length > 0) {
        ...
        collection.forEach(function (className) {
          ...
          className.locations.forEach(function (location) {
            if (location.file) {
              report.push('  ' + chalk.grey(location.file + (location.lineNumber ? ':' + location.lineNumber : '')))
            }
          })
        })
      }
      ...
      case 'rule':
        if (obj.classNames) {
          obj.classNames.forEach(function (className) {
            pushUnique(cssClasses, className)
          })
        }
  ...
  function pushUnique(collection, value, file, lineNumber) {

    var classNames = value.replace(/\'/g, '').split(' ')
      , location = { 'file': file, 'lineNumber': lineNumber }
