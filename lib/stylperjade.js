...
function process(cssFiles, jadeFiles, options, cb) {
  ...
  function loadOptions(cssFiles, jadeFiles, options) {
    ...
    // TBC: if options is not null use it
    // if it is go ahead and find a config file

    if (options.stylperjaderc) {
      options = parseConfig(options.stylperjaderc)
    } else {
      // Try and load config from root of project, or home directory
      var stylperjadercPath = null
  ...
  function parseConfig(path) {
    ...
    try {
      return options = JSON.parse(fs.readFileSync(path, 'utf-8'))
    } catch (err) {
