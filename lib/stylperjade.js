  ...
  async.each(cssFiles, processCssFile, function (err) {
    ...
    async.each(jadeFiles, processJadeFile, function (err) {
      ...
      var cssBlacklisted = filterBlacklistedClasses(cssClasses, options.cssBlacklist)
        ...
        , hasBlacklisted = blacklistedCount > 0
      ...
      if (hasBlacklisted) {
        report.push(chalk.red('✘ Stylperjade: ' + blacklistedCount + ' Blacklisted classes found'))

        if (cssBlacklisted.length > 0) {
          report.push('')
          report.push(chalk.cyan(cssBlacklisted.length + ' blacklisted CSS classes:'))
          cssBlacklisted.forEach(function (blacklisted) {
            report.push('- ' + blacklisted)
          })
        }

        if (jadeBlacklisted.length > 0) {
          report.push('')
          report.push(chalk.cyan(jadeBlacklisted.length + ' blacklisted Jade classes:'))
          jadeBlacklisted.forEach(function (blacklisted) {
            report.push('- ' + blacklisted)
          })
        }
      } else {
        report.push(chalk.green('✔ Stylperjade: No blacklisted classes found'))
      }
